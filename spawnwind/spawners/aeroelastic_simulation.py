...
class AeroelasticSimulationSpawner(TaskSpawner):
    ...
    simulation_time = FloatProperty(doc='Simulation time in seconds, excluding time before start of output', abstract=True)
    ...
    # manoeuvres
    pitch_manoeuvre_time = FloatProperty(doc='Time in seconds at which all blades start a pitch manoeuvre', abstract=True)
    blade_pitch_manoeuvre_time = ArrayProperty(doc='Time in seconds at which one particular blade starts a pitch manoeuvre', abstract=True, type_=float)
    ...
    # faults
    grid_loss_time = FloatProperty(doc='Sets time in seconds for grid loss (zero generator torque) to occur', abstract=True)
    ...
    wind_file = StringProperty(doc='Directly set the wind file for use in simulation. This wind file must be externally generated before running tasks', abstract=True)
